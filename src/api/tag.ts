...
export const editTag = (data?: object) => {
  return http.request<TagResult>("post", "/api/tag/update", { data });
};

/** 删除标签 */
export const deleteTagList = (data?: object) => {
  return http.request<TagResult>("delete", "/api/tag/delete", { data });
};

/** 获取标签列表 */
export const getTagDictionary = () => {
  return http.request<TagResult>("post", "/api/tag/getTagDictionary", {});
};
